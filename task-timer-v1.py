...
def summarize_time():
    ...
    with open(DATA_FILE, newline='') as f:
        reader = csv.reader(f)
        for row in reader:
            if len(row) != 5: continue  # tránh lỗi nếu file bị hỏng dòng
...
class TaskTimerApp:
    def __init__(self, root):
        ...
        self.summary_label = tk.Label(root, text=summarize_time(), justify='left')
    ...
    def stop_timer(self):
        ...
        task = self.task_var.get().strip() or "Unnamed Task"
